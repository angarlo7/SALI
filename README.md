...
- **FX Rates**: Live USD-to-EUR/MXN rates are fetched at runtime from the European Central Bank via the Frankfurter API (`api.frankfurter.dev`). The hardcoded values in `app.js` are fallbacks used only if the live fetch fails.
